...
export function FacultyProfile() {
			...
			if (snapshot.exists()) {
				...
				getDownloadURL(storageRef(storage, `avatars/${currentUser.uid}/${snapshot.val().photoUrl}`))
					.then((url) => {
                        ...
                        const avatar1 = document.getElementById(`user-profile-dp`)
						avatar1.setAttribute('src', url)
					}).catch((err) => {
  ...
  {/* <div class={ProfileStyle.search_box}>
      ...
      <input type="text" placeholder="Search Here..."/>
          ...
          <input type="text" class={`${ProfileStyle.searchbar} form-control`} placeholder="Seach here.." aria-label="Recipient's username" aria-describedby="button-addon2"/>
          ...
          <img id = {`user-profile-avatar`} src="../images/ayaka.jpg" alt="" class={ProfileStyle.profile}/>
          </div>
